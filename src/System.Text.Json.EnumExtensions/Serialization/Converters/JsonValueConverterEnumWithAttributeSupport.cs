    ...
    where T : struct, Enum
{
    ...
    private readonly string? _negativeSign = (int)EnumTypeCode % 2 == 0 ? null : NumberFormatInfo.CurrentInfo.NegativeSign;
    ...
    public JsonValueConverterEnumWithAttributeSupport(EnumConverterOptions options, JsonNamingPolicy? namingPolicy = null)
    {
    ...
    private bool IsValidIdentifier(string value)
    {
        ...
        // if needed.
        return value[0] >= 'A' && (_negativeSign == null || !value.StartsWith(_negativeSign));
    }
    ...
    private bool TryParseFromAttribute(string? value, out T resolvedEnumValue)
    {
        ...
        {
            if (TryGetCustomAttributeValue(enumValue.ToString(), out var attributeValue) && string.Equals(value, attributeValue, StringComparison.OrdinalIgnoreCase))
            {
    ...
    private bool TryGetCustomAttributeValue(string enumValueAsString, [NotNullWhen(true)] out string? attributeValue)
    {
        attributeValue = null;
